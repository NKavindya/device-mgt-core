...
public class NotificationArchivalDAOTests extends BaseNotificationManagementTest {
    ...
    public void testMoveNotificationsToArchiveByConfig() throws Exception {
        ...
        when(archivalDAO.moveNotificationsToArchiveByConfig(any(), eq(TENANT_ID), eq(CONFIG_ID)))
                .thenReturn(expectedIds);
        List<Integer> archived = archivalDAO.moveNotificationsToArchiveByConfig(
                new Timestamp(System.currentTimeMillis()), TENANT_ID, CONFIG_ID);
        Assert.assertEquals(archived, expectedIds);
        verify(archivalDAO, times(1))
                .moveNotificationsToArchiveByConfig(any(), eq(TENANT_ID), eq(CONFIG_ID));
    ...
    public void testMoveNotificationsToArchiveExcludingConfigs() throws Exception {
        ...
        List<Integer> expectedIds = List.of(dummyNotificationId);
        when(archivalDAO.moveNotificationsToArchiveExcludingConfigs(any(), eq(TENANT_ID), eq(excluded)))
                .thenReturn(expectedIds);
        List<Integer> result = archivalDAO.moveNotificationsToArchiveExcludingConfigs(
                new Timestamp(System.currentTimeMillis()), TENANT_ID, excluded);
        Assert.assertEquals(result, expectedIds);
        verify(archivalDAO, times(1))
                .moveNotificationsToArchiveExcludingConfigs(any(), eq(TENANT_ID), eq(excluded));
    ...
    public void testDeleteOldNotificationsByConfig() throws Exception {
        when(archivalDAO.deleteOldNotificationsByConfig(any(), eq(TENANT_ID), eq(CONFIG_ID)))
                .thenReturn(1);
        int deleted = archivalDAO.deleteOldNotificationsByConfig(
                new Timestamp(System.currentTimeMillis()), TENANT_ID, CONFIG_ID);
        Assert.assertEquals(deleted, 1);
        verify(archivalDAO, times(1))
                .deleteOldNotificationsByConfig(any(), eq(TENANT_ID), eq(CONFIG_ID));
        log.info("Deleted old notifications by config.");
    }

    @Test
    public void testDeleteExpiredArchivedNotifications() throws Exception {
        doNothing().when(archivalDAO).deleteExpiredArchivedNotifications(any(), eq(TENANT_ID));
        archivalDAO.deleteExpiredArchivedNotifications(new Timestamp(System.currentTimeMillis()), TENANT_ID);
        verify(archivalDAO, times(1))
                .deleteExpiredArchivedNotifications(any(), eq(TENANT_ID));
