...
public class NotificationHelper {
    ...
    private static String getTenantDomain(int tenantId) {
        try {
            ...
            log.debug("Failed to resolve tenant domain for tenantId: " + tenantId + ". Using carbon.super.", e);
    ...
    private static String stripTenantDomainIfMatches(String username, String tenantDomain) {
        ...
        String u = username.trim();
        if (u.isEmpty() || tenantDomain == null || tenantDomain.trim().isEmpty()) {
            return u;
        }
        ...
        if (Constants.SUPER_TENANT_DOMAIN.equals(tenantDomain) && u.endsWith("@carbon.super")) {
            return u.substring(0, u.length() - "@carbon.super".length());
        }
        return u;
    }
}
