...
import io.entgra.device.mgt.core.device.mgt.core.internal.DeviceManagementStartupHandler;
...
//            }
    ...
    public List<DeviceFeatureInfo> getDeviceFeatureOperations() throws DeviceFeatureOperationException {
        ...
        try {
            if (deviceTypeMetaDefinition == null || deviceTypeMetaDefinition.getFeatures() == null) {
                log.warn("No device features available for device type: " + getType());
                return featureList;
            }
            for (Feature feature : deviceTypeMetaDefinition.getFeatures()) {
                if (FEATURE_TYPE_OPERATION.equals(feature.getType())) {
                    ...
                    featureInfo.setDeviceType(feature.getDeviceType());
                    featureList.add(featureInfo);
                }
            }
            DeviceFeatureOperationDAO deviceFeatureOperationDAO = new GenericDeviceFeatureOperationDAOImpl();
            DeviceManagementDAOFactory.openConnection();
            deviceFeatureOperationDAO.updateDeviceFeatureDetails(featureList);
        } catch (Exception e) {
            String msg = "An error occurred while retrieving and updating device feature operations.";
            log.error(msg, e);
