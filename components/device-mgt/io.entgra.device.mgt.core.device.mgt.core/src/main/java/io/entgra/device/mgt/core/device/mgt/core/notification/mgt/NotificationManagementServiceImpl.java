    ...
    public void handleOperationNotificationIfApplicable(Operation operation, Map<Integer, Device> enrolments, int tenantId)
        ...
        try {
            ...
            if (config != null) {
                ...
                // user-specific
                if (config.getRecipients() != null && config.getRecipients().has("users")) {
                    JSONArray users = config.getRecipients().getJSONArray("users");
                    for (int i = 0; i < users.length(); i++) {
                        usernames.add(users.getString(i));
                    }
                }
                // role-based
                if (config.getRecipients() != null && config.getRecipients().has("roles")) {
                    JSONArray roles = config.getRecipients().getJSONArray("roles");
